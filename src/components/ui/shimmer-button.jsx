  ...
  return (
    (<button
      ...
      className={cn(
        ...
        "tw-transform-gpu tw-transition-transform tw-duration-300 tw-ease-in-out active:tw-translate-y-px",
        className
      )}
      ref={ref}
      {...props}>
      ...
      <div
        ...
        {/* spark */}
        <div
          className="tw-absolute tw-inset-0 tw-h-[100cqh] tw-animate-shimmer-slide [aspect-ratio:tw-1] [border-radius:tw-0] [mask:tw-none]">
          {/* spark before */}
          <div
            className="tw-animate-spin-around tw-absolute tw--inset-full tw-w-auto tw-rotate-0 [background:tw-conic-gradient(from_calc(270deg-(var(--spread)*0.5)),transparent_0,var(--shimmer-color)_var(--spread),transparent_var(--spread))] [translate:tw-0_0]" />
        </div>
      </div>
      {children}
      {/* Highlight */}
      <div
        className={cn(
          "tw-insert-0 tw-absolute tw-size-full",
